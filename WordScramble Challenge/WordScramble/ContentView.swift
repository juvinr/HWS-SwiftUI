...
struct ContentView: View {
    ...
    func addNewWord() {
        ...
        guard answer.count > 2 else {
            wordError(title: "Word too short", message: "Enter a word with atleast three letters")
            return
        }
        
        guard answer != rootWord else {
            wordError(title: "Word is same as root word", message: "Try to use a new word")
            return
        }
        ...
        if usedWords.count == 4 || usedWords.count == 9 {
            userScore += 50
        }
